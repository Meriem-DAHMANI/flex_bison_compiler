...
int recherchePos(char entite[])
 ...
 //definir une focntion qui detecte la double declaration
  int doubleDeclaration(char entite[])
  {
  int pos;
  pos=recherchePos(entite);
  if(strcmp(tab[pos].type,"")==0) return 0;
     else return -1;
     
  }

//définir une focntion pour inserer le type
...
//vérifier si la variable est déclarée
  int nonDeclaree(char entite[])
  {
    int pos;
  pos=recherchePos(entite);
  if(recherchePos(entite)==-1) return 0;
     else return -1;
  }
